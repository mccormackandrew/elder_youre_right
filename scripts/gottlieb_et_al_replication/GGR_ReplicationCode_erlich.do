...
* Create coarsened_age dummy variables
tab coarsened_age, generate(coarsened)
...
* Using SVY/SUEST (with age difference)
svyset URBRUR [pweight=Withinwt], strata(REGION) singleunit(certainty)
svydescribe
...
foreach y in $outcomes {
svy: reg `y' female $min_controls ib3.coarsened_age_cat##i.female age i.ccodecow i.ROUND, dof(5) 
est sto `y'
}
...
*create regression table with age difference
esttab Economy Poverty Infrastructure Water Agriculture using "suest_main1_age.tex", star(* 0.10 ** 0.05 *** 0.01) se b(3) se(3) r2(2) label booktabs alignment(D{.}{.}{-1}) replace ///
	keep(female coarsened_age_cat age $min_controls) stats(N,layout("\multicolumn{1}{c}{@}") labels("Observations") f(0)) ///
	title(Impact of Gender on the Likelihood of Policy Domain Prioritization\label{tab:suest1})  ///
	nonotes ///
	addnote("Pooled seemingly unrelated regression analyses with country fixed effects." "Standard errors are in parentheses. $^* p<0.10$, $^{**} p<0.05$, $^{***} p<0.01$")
	
esttab Violence Health Rights  Education Services using "suest_main2_age.tex", star(* 0.10 ** 0.05 *** 0.01) se b(3) se(3) r2(2)   label booktabs alignment(D{.}{.}{-1}) replace ///
	keep(female coarsened_age_cat age $min_controls) stats(N,layout("\multicolumn{1}{c}{@}") labels("Observations") f(0)) ///
	title(Impact of Gender on the Likelihood of Policy Domain Prioritization\label{tab:suest2})  ///
	nonotes ///
	sub("Social/Political Rights" "Rights") ///
